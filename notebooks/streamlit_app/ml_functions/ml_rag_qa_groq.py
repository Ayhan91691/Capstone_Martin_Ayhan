...
def search_rag(
    ...
    dataset_size = len(df)
    candidate_pool = min(max(k * candidate_multiplier, min_candidate_pool), dataset_size)
    ...
    used_result = result.head(k).reset_index(drop=True)

    return used_result, dataset_size, candidate_pool, retrieved_count
...
def ask_rag_question_groq(
    ...
    docs, dataset_size, candidate_pool, retrieved_count = search_rag(
        rag_dir=rag_dir,
        query=question,
        k=k,
