...
def _build_context(docs: pd.DataFrame) -> str:
    ...
    for i, (_, row) in enumerate(docs.iterrows(), start=1):
        text = _shorten_text(row.get("content", ""), max_chars=180)
        score = row.get("score", "")
...
def _generate_text(generator, prompt: str, max_new_tokens: int = 120) -> str:
...
Task:
- Write 3 short bullet points summarizing the main problems.
- Create exactly 3 complaint categories.
...
""".strip()
    ...
    part_1 = _generate_text(generator, prompt_1, max_new_tokens=120)

    prompt_2 = f"""
You are analyzing app feedback.

Based on this intermediate analysis:
{part_1}

Now do this:
- For each of the 3 categories, suggest 2 concrete solutions.
- Suggest 3 realistic new app features.
...
""".strip()
    ...
    part_2 = _generate_text(generator, prompt_2, max_new_tokens=160)
