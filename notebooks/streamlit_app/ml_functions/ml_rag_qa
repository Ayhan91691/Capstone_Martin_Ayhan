...
def save_analysis_outputs(
    ...
    output_root = rag_dir / "analysis_outputs"
    ...
    sampled_reviews_path = output_dir / "sampled_reviews.csv"
    sampled_df.to_csv(sampled_reviews_path, index=False, encoding="utf-8-sig")

    json_path = output_dir / "analysis_result.json"
    md_path = output_dir / "analysis_result.md"
    ...
    result_payload = {
        ...
        "sampled_reviews_file": str(sampled_reviews_path),
        "sections": {
            "problems_and_categories": part_1,
            "solutions_features_and_general_ideas": part_2,
        },
    ...
    print(f"💾 JSON gespeichert: {json_path}", flush=True)
    print(f"💾 Markdown gespeichert: {md_path}", flush=True)
    print(f"💾 Sampled Reviews gespeichert: {sampled_reviews_path}", flush=True)

    return {
        "output_dir": output_dir,
        "json_file": json_path,
        "markdown_file": md_path,
        "sampled_reviews_file": sampled_reviews_path,
    }
...
2. For each category, suggest 1 to 2 category-specific feature ideas.
3. At the end, suggest 3 to 5 general app ideas that would make the overall app better.
4. At the end, suggest 3 to 5 technical / programming recommendations that would be sensible for improving the app architecture, reliability, or performance.
...
## Category-Specific Feature Ideas
### Category 1: <label>
- ...
- ...
...
## Technical / Programming Suggestions
- ...
- ...
- ...
...
- If the analysis contains only 3 or 4 categories, only return content for those categories.

Analysis:
{part_1}
[/INST]"""

    part_2 = _generate_mistral(generator, prompt_2, max_new_tokens=360)
...
## Solutions, Feature Ideas, and General Recommendations
...
if __name__ == "__main__":
    ...
    result, sampled_reviews, saved_files = ask_rag_clustered(
        rag_dir=rag_dir,
